...
export ARTISAN="php ${DEPLOY_SSH_PATH}/current/artisan"
...
function ssh-exec-silent() {
    ssh -o StrictHostKeyChecking=no -i ${LOCAL_SSH_KEY} -p ${DEPLOY_SSH_PORT} ${DEPLOY_SSH_USER}@${DEPLOY_SSH_IP} "echo \$(bash -c \"${1}\")"
}
...
function ssh-cp() {
    cmd-exec "$1" "scp -o StrictHostKeyChecking=no -i ${LOCAL_SSH_KEY} -P ${DEPLOY_SSH_PORT} ${2} ${DEPLOY_SSH_USER}@${DEPLOY_SSH_IP}:${3}" "${4}" "${5}" "finish_script"
}
