<template>
    <div class="universal-report__form">
        <div class="data-entry controls-row">
            <Calendar class="controls-row__item" sessionStorageKey="amazingcat.session.storage.universalreport"
            ...
            <ExportDropdown class="export-btn dropdown controls-row__btn controls-row__item" position="left-top"
                trigger="hover" @export="onExport" />
        </div>
        <validation-provider v-slot="{ errors }" :rules="'required'" :name="$t('field.name')">
            <at-input v-model="reportName" class="data-entry" :placeholder="$t('field.name')" :type="'text'"
                :status="errors.length > 0 ? 'error' : ''"></at-input>
            <small>{{ errors[0] }}</small>
        </validation-provider>
        <at-select class="data-entry" :value="base" @on-change="changeBase">
            <at-option v-for="base in bases" :key="base" :value="base"
                :label="$t(`field.data-objects.${base}.1`)"></at-option>
        </at-select>
        <div v-if="base">
            <label>{{ $t(`field.data-objects.${base}.1`) }}</label>
        </div>
        <obj-data-select class="data-entry" :options="dataObjects" :base="selectedBase"
            :selectedOptions="selectedDataObjects" @on-change="change" />
        <div v-if="base">
            <label>{{ $t(`field.data-objects.${base}.2`) }}</label>
        </div>
        <fields-select class="data-entry" localePath="field.fields" :options="fields" :selectedOptions="selectedFields"
            @on-change="onFieldsChange" />
        <div v-if="base">
            <label>{{ $t(`field.data-objects.${base}.3`) }}</label>
        </div>
        <v-select class="data-entry" :localePath="`field.fields.${selectedBase}.charts`" :options="charts"
            :selectedOptions="selectedCharts" @on-change="onChartsChange" />
        <div class="controls-row">
            ...
            <at-button class="controls-row__item" type="success">
                <router-link class="link" :to="{ name: 'report.universal.view', params: { id: this.$route.params.id } }">
