<template>
    <div class="universal-report__form">
        ...
        <at-select class="data-entry" :value="main" @on-change="changeMain">
            <at-option v-for="main in mains" :key="main" :value="main"
                :label="$t(`field.data-objects.${main}.1`)"></at-option>
        </at-select>
        <div v-if="main">
            <label>{{ $t(`field.data-objects.${main}.1`) }}</label>
        </div>
        <obj-data-select class="data-entry" :options="dataObjects" :main="selectedMain"
        ...
        <v-select class="data-entry" :localePath="`field.fields.${selectedMain}.charts`" :options="charts"
